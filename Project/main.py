    ...
    with tf.Session() as sess:
        ...
        while evaluated_images != img_number:
            ...
            for idx,current_id in enumerate(test_id):
                '''
                #b case
                if current_proba[idx,label_per_track[current_id]] in sorted(current_proba[idx],reverse=True)[1:3] and current_proba[idx,label_per_track[current_id]] != 0:
                    print(current_proba[idx])
                    print(label_per_track[current_id])
                    print(test_tag[idx])
                    print(batch_count)
                ...
                '''
                #c case
                if max(current_proba[idx])<0.5 and current_argmax[idx] == label_per_track[current_id]:
                    print(current_proba[idx])
                    print(label_per_track[current_id])
                    print(test_tag[idx])
                    print(batch_count)
        ...
        for i in range(len(max_proba)):
            if max_proba[i] == label_per_track[i] and i>10:
                if not all(x == label_per_track[i] for x in segment_predic[i]):
                    track_chosen = i
                    break
